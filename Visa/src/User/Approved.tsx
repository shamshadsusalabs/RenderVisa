...
export default function ApprovedVisaTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`https://rendervisa.onrender.com/api/VisaApplication/approved/${phoneNumber}`);
  ...
  const filteredData = data
    ...
    .filter(item => item.visaId.toLowerCase().includes(countryFilter.toLowerCase()))
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto min-h-screen">
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Approved Visa Applications</h1>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <input
          type="text"
          placeholder="Search by Visa ID"
          className="p-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm"
          value={countryFilter}
      ...
      <div className="bg-white rounded-2xl shadow-lg border overflow-hidden">
        ...
        {filteredData.length === 0 && (
          <div className="p-8 text-center text-gray-500">
            {data.length === 0 ? 'No visa applications found' : 'No approved applications match your filters'}
          </div>
        )}
      </div>
